...
import { setHandoffMode } from "@/lib/human-handoff";
...
export async function POST(req: Request) {
  try {
    ...
    const shouldActivateHandoff = body?.activateHandoff !== false;
      ...
      await setHandoffMode({
        ...
        reason:
          typeof body?.reason === "string"
            ? body.reason
            : "Encaminhado para atendimento humano via integracao n8n.",
        actor:
    ...
    const messageText =
      botSettings?.humanHandoffMessage?.trim() ||
    ...
    return NextResponse.json({
      ...
      number,
      message: messageText,
      handoffActivated: shouldActivateHandoff,
      details: providerPayload,
    });
  } catch (error) {
    console.error("n8n humano message route error:", error);
    return NextResponse.json(
      {
        result: "ERROR",
        error: "Falha ao enviar mensagem de encaminhamento para humano.",
      },
      { status: 500 },
    );
  }
